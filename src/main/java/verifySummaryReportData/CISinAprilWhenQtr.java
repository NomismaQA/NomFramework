...
public class CISinAprilWhenQtr extends BasePage
{
	int i;
	int size;
	int j=4;
...
double CISSufferedValue=0;
	...
	public CISinAprilWhenQtr (WebDriver driver)
	{
		super(driver);
	}
...
//	2						    	assertEquals(BalancecarryAmount, Expectedcarry , "balance carry did not matched");
...
//	3						    	assertEquals(BalanceOwedAmount, prevTax);
//	4						    	assertEquals(BalancecarryAmount, Expectedcarry);
...
//	5						    	assertEquals(BalanceOwedAmount, prevBalowed);
//	6						    	assertEquals(BalancecarryAmount, Expectedcarry);
...
//   1 	    							assertEquals(Totalamount, PAYE_NIValue);
    ...
    */
        ...
        public void CISforQtrinApr() throws InterruptedException
        	    	{
...
//  1  							    	assertEquals(BalanceOwedAmount, ExpectedBalanceOwed ,"Balanced owed not matched");    //verifying balance owed for april
    							    	...
    							    	Expectedcarry=(double) (Math.round(Expectedcarry *100)/100.0);
    							    	System.out.println("total tax+="+Expectedcarry);
//  2  							    	assertEquals(BalancecarryAmount, Expectedcarry , "balance carry did not matched");    //verifying C/F value in summary report
    							    	prevBalowed=BalancecarryAmount;
    							    	
    							    	if(CISSufferedValue==0)
    							    	{
    							    		
//    1							    		assertEquals(Totalamount, 0);
//    							    		
//    1							    		assertEquals(0, PAYE_NIValue);
        	         		  	...
        	         		  	//	Totaltax=Totaltax-CISDeducted;
    	         		  			...
    	         		  			if(CISSufferedValue>Totaltax)
   	        			  		 {
        	         		  		...
        	         		  		System.out.println("Balance owed=="+BalanceOwedAmount);
        	         		  		System.out.println("prevBalance=="+prevBalowed);
//   2     	         		  		assertEquals(BalanceOwedAmount, prevBalowed ,"Balanced owed not matched.");    //verifying balance owed
        	         		  		
        	         		  		System.out.println("prevBalowed+="+prevBalowed);
        	         		  		System.out.println("Tax="+Totaltax);
        	         		  		double Expectedcarry=Totaltax+prevBalowed;
        	         		  		System.out.println("Tax="+Expectedcarry);
							    	Expectedcarry=(double) (Math.round(Expectedcarry *100)/100.0);
							    	System.out.println("Expectedcarry tax+="+Expectedcarry);
// 2							    	assertEquals(BalancecarryAmount, Expectedcarry , "balance carry did not matched");    //verifying C/F value in summary report
							    	prevBalowed=BalancecarryAmount;
...
//2							    	assertEquals(Totalamount, PAYE_NIValue);
        	         		  	}
